...
export const UpdateOs = () => {
  ...
  const [osUpdateselect, setOsUpdate] = useState(
    indexOS?.data?.succes?.filter((cliente) => cliente.id === idUpdate)
  );
  ...
  let usuario = usuarios?.data?.succes;
  return (
    <section className="md:w-1/4 w-full h-2/3 bg-white rounded-xl shadow-xl border-2 p-2 flex flex-col  overflow-hidden animate__animated animate__fadeInDown">
      ...
      >
        ...
        >
          {clientes?.data?.succes?.map((cliente) => (
            <option
              value={cliente.id}
              selected={
                osUpdateselect[0]?.id_cliente === cliente.id ? true : false
              }
              key={cliente.id}
            >
              {cliente.nombre}
        ...
        >
          {estados?.data?.succes?.map((estado) => (
            <option
              value={estado.id}
              selected={
                osUpdateselect[0]?.id_estado === estado.id ? true : false
              }
              key={estado.id}
            >
              {estado.nombre}
        ...
        >
          <option
            value={osUpdateselect[0]?.use_id}
            selected
            key={osUpdateselect[0]?.use_id}
          >
            {osUpdateselect[0]?.consultor}
          </option>
